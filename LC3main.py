...
def run_instructions():
    while (memory[MCR] >> 15) & 0b1 == 1:
        ...
        regs.IR = inst
        # print to_hex_string(regs.PC - 1) + " " + to_hex_string(regs.IR) + " " + to_hex_string(regs.CC) + " " +
        # to_hex_string(regs.registers[0])
        handle_instruction(inst)
        if not ON and regs.PC == sign_extend(0xFD79, 16):
            memory[MCR] = 0x7FFF
